  ...
  it('renders each card, observed states, unknown gaps and a restrictive CSP', () => {
    ...
    const html = renderIdleHistory({
      ...
      cards: [{ id: '0', state: 'idle', observedAt: now, idleSince: now - 60000,
        bins: [[now - 2 * HISTORY_BUCKET_MS, 'busy'], [now - HISTORY_BUCKET_MS, 'mixed'], [now, 'idle']] }],
    }, now, 'nonce');
    ...
    expect(html).toContain('<span class="cell mixed"');
    expect(html).toContain('<i class="cell mixed"></i>');
    expect(html).toContain('.cell.mixed{background:repeating-linear-gradient(135deg,var(--vscode-testing-iconPassed) 0 4px,var(--vscode-charts-yellow) 4px 8px)}');
